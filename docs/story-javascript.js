...
window.ME = window.ME || {};
ME.resolveWsUrl = function() {
  var params = new URLSearchParams(window.location.search);
  var override = params.get('ws') || window.ME_BACKEND_WS;
  if (override) return override;

  // Keep local TouchDesigner workflow working by default.
  if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    return 'ws://localhost:9980';
  }

  // Default production backend hosted on Render.
  return 'wss://embodied-interaction.onrender.com/ws?role=browser';
};
ME.wsUrl = ME.resolveWsUrl();
ME.ws = null;
ME.totalPassages = 10;
ME.currentBess = null;
ME.targetBess = null;
...
ME.SFX_MAP = {
  ...
  4:  [{ f: 'audio/sfx/sfx_clock_ticking.mp3', at: 0 },
       { f: 'audio/sfx/sfx_clock_strike.mp3', at: -12 }],
...
ME.CHAR_MAP = {
  1: [], 2: [], 3: [],
  4:  [{ f: 'audio/characters/char_knock_door.mp3', at: -10 },
