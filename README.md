...
The project extracts Laban Effort Action Drives from Kafka's The Metamorphosis by conducting a reading and a full BESS (Body, Effort, Shape, Space) annotation of key passages. I then convert these qualities into generative audiovisuals using TouchDesigner and ComfyUI. The pipeline inverts Fdili Alaoui et al. (2017): rather than body → sensors → Effort classification, the flow is text → reading → BESS annotation → generative audiovisual output. Based on de Meijer's (1989) empirical Effort-to-emotion mapping, the passages are chosen for a five-minute prototype that covers Action Drives. The TouchDesigner implementation uses a multi-layer visual architecture together with multi-layer ElevenLabs audio.
